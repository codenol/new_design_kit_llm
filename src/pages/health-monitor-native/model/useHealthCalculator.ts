...
import { MetricValues, createInitialMetrics, tickMetrics } from "./mockMetrics";
import { evaluateStatus, worstStatus } from "./healthRules";
...
function computeHealth(config: Config, metrics: MetricValues): ContourHealth {
  const pakHealthList: PAKHealth[] = config.contour.paks.map((pak) => {
    const groupHealthList: GroupHealth[] = pak.groups.map((group) => {
      const nodeHealthList: NodeHealth[] = group.nodes.map((node) => {
        const indicatorHealthList: TopLevelIndicatorHealth[] = node.indicators.map(
          (indicator) => {
            const subHealthList: LowLevelIndicatorHealth[] =
        ...
        return {
          ...
          status,
          indicators: indicatorHealthList,
        };
      });
  ...
  const contourStatus: HealthStatus = worstStatus(pakHealthList.map((p) => p.status));
...
export function useHealthCalculator(config: Config) {
  const [metrics, setMetrics] = useState<MetricValues>(() =>
    createInitialMetrics(config),
  );

  useEffect(() => {
    setMetrics(createInitialMetrics(config));
  }, [config]);
