...
function parseLowLevel(raw: Record<string, unknown>): LowLevelIndicator {
  return {
    ...
    rules: {
      warning: raw.rules
        ? String((raw.rules as Record<string, unknown>).warning ?? "") ||
          undefined
        : undefined,
      critical: raw.rules
        ? String((raw.rules as Record<string, unknown>).critical ?? "") ||
          undefined
        : undefined,
    },
  };
...
function parseIndicator(raw: Record<string, unknown>): TopLevelIndicator {
  return {
    ...
    name: String(raw.name ?? ""),
    sub_indicators: (
      (raw.sub_indicators as Record<string, unknown>[] | undefined) ?? []
    ).map(parseLowLevel),
  };
}
...
function parseNode(raw: Record<string, unknown>): ConfigNode {
  return {
    ...
    indicators: ((raw.indicators as Record<string, unknown>[] | undefined) ?? []).map(
      parseIndicator,
    ),
  };
}
...
function parsePAK(raw: Record<string, unknown>): PAK {
  return {
    ...
    groups: ((raw.groups as Record<string, unknown>[] | undefined) ?? []).map(
      parseGroup,
    ),
  };
}
...
function parseContour(raw: Record<string, unknown>): Contour {
  return {
    name: String(raw.name ?? ""),
    paks: ((raw.paks as Record<string, unknown>[] | undefined) ?? []).map(
      parsePAK,
    ),
  };
}
