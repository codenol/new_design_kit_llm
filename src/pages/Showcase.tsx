...
const wizardSteps = [
  { name: 'Шаг 1', isActive: true, isDone: false, content: <p>Контент первого шага.</p> },
  { name: 'Шаг 2', isActive: false, isDone: false, content: <p>Контент второго шага.</p> },
  { name: 'Шаг 3', isActive: false, isDone: false, content: <p>Контент третьего шага.</p> },
];
    ...
    <AppLayout breadcrumbTrail={showcaseBreadcrumbTrail}>
      <h1 style={{ marginTop: 0 }}>Витрина компонентов</h1>
          ...
          <section className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Кнопки, поля, переключатели</h2>
            <div className="flex flex-wrap align-items-center gap-2 mb-3">
              ...
              <Dropdown
                ...
                className="w-12rem p-inputtext-sm"
              />
              <Button label="Основная кнопка" className="p-button-pr" size="small" />
              <Button label="Вторичная" className="p-button-outlined" size="small" />
              <Button icon="pi pi-download" className="p-button-rounded p-button-outlined" size="small" />
              <Button icon="pi pi-cog" className="p-button-rounded p-button-outlined" size="small" />
              <div className="flex align-items-center gap-2">
                <InputSwitch checked={switchChecked} onChange={(e) => setSwitchChecked(e.value ?? false)} />
          ...
          <section className="mb-4">
            ...
            <Button label="Открыть модалку" onClick={() => setModalVisible(true)} className="mb-2" />
            <Dialog
            ...
            >
              <p>Контент модального окна. Закройте по крестику или клику по фону.</p>
            </Dialog>
          </section>
          ...
          <section className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                label="Info"
                size="small"
                onClick={() => showToast({ severity: 'info', summary: 'Инфо', detail: 'Сообщение информационного характера' })}
              />
              <Button
                label="Успех"
                size="small"
                onClick={() => showToast({ severity: 'success', summary: 'Готово', detail: 'Операция выполнена успешно' })}
              />
              <Button
                label="Предупреждение"
                size="small"
                onClick={() => showToast({ severity: 'warn', summary: 'Внимание', detail: 'Проверьте введённые данные' })}
              />
              <Button
                label="Ошибка"
                size="small"
                className="p-button-danger"
                onClick={() => showToast({ severity: 'error', summary: 'Ошибка', detail: 'Что-то пошло не так' })}
              />
            </div>
          </section>
          ...
          <section className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Messages (список сообщений)</h2>
            <div className="flex flex-wrap gap-2 mb-2">
              <Button
                label="Показать info"
                size="small"
                onClick={() => showMessages({ severity: 'info', text: 'Информационное сообщение в блоке.' })}
              />
              <Button
                label="Показать success"
                size="small"
                onClick={() => showMessages({ severity: 'success', text: 'Успешное сообщение.' })}
              />
              <Button
                label="Показать warn"
                size="small"
                onClick={() => showMessages({ severity: 'warn', text: 'Предупреждение.' })}
              />
              <Button
                label="Очистить"
                size="small"
          ...
          <section className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Таблица (клик по строке открывает Drawer)</h2>
            <div className="card" style={{ minWidth: 0, overflow: 'auto', padding: 0 }}>
              <DataTable
                value={demoTableRows}
                selection={tableSelection}
                onSelectionChange={(e) => setTableSelection(Array.isArray(e.value) ? e.value : (e.value ? [e.value] : []))}
                  ...
                  body={(row: DemoTableRow) => (
                    <span
                      className={classNames(
                        ...
                        row.category === 'Служебные' && 'status-badge--not-available'
                      )}
                    >
                      {row.category}
                    </span>
          ...
          <Drawer
            ...
            onHide={() => { setDrawerVisible(false); setDrawerRow(null); }}
            header={drawerRow ? `Детали: ${drawerRow.name}` : 'Детали'}
            position="right"
          >
            {drawerRow && (
              <div className="flex flex-column gap-2">
                <p><strong>ID:</strong> <span className="text-color-secondary">{drawerRow.id}</span></p>
                <p><strong>Название:</strong> <span className="text-color-secondary">{drawerRow.name}</span></p>
                <p><strong>Версия:</strong> <span className="text-color-secondary">{drawerRow.version}</span></p>
                <p><strong>Статус:</strong> <span className="text-color-secondary">{drawerRow.statusName} ({drawerRow.statusCode})</span></p>
                <p><strong>Категория:</strong> <span className="text-color-secondary">{drawerRow.category}</span></p>
              </div>
            )}
          </Drawer>
    </AppLayout>
