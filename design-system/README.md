# Venom UI React Sandbox

Приложение на React.
Песочница для работы: состоит из дизайн-системы (токены, layout, переопределения PrimeReact, uikit) и страниц, которые создаёт UXUI дизайнер с помощью промптов к LLM.

## Установка (если просят собери проект, а до этого сборок не было)
